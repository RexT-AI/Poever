...
/**
 * json 파일로부터 predictions 반환
 * @param {JSON} json API에서 받아온 json 값
 * @returns JSON.predictions
 */
export function getPred(json) {
  if (json.length === 0) {
    return undefined;
  }
...
export function func1(pred, side) {
  ...
  if (side == "right") {
    ...
    if (
      right_hip < SCORE_THRESHOLD ||
      nose < SCORE_THRESHOLD ||
      right_ear < SCORE_THRESHOLD
    ) {
      return undefined;
    }
  ...
  if (
    left_ear < SCORE_THRESHOLD ||
    nose < SCORE_THRESHOLD ||
    left_hip < SCORE_THRESHOLD
  ) {
    return undefined;
  }
...
export function func2(pred, side) {
  ...
  if (side == "right") {
    ...
    if (
      right_knee < SCORE_THRESHOLD ||
      right_hip < SCORE_THRESHOLD ||
      neck < SCORE_THRESHOLD
    ) {
      return undefined;
    }
...
export function func3(pred, side) {
  ...
  if (side == "right") {
    ...
    if (
      right_knee === undefined ||
      left_knee.score > right_knee + CMP_THRESHOLD
    ) {
      return true;
    }
...
/**
 * json으로부터 이미지의 고개(0), 허리(1), 무릎(3) 각과 다리 꼬기 여부(2)를 배열로 반환
 * @param {JSON} json
 * @returns
 */
export function getPose(json) {
  const pred = getPred(json); // predictions 가져오기
  if (pred === undefined) return undefined;
  const side = getSide(pred); // 앉은 방향 가져오기
  if (side === undefined) return undefined;

  var res1 = true,
    res2 = true,
    res3 = func3(pred, side),
    res4 = true;
