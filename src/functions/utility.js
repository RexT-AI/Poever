const SCORE_THRESHOLD = 0.3; // 각도 계산 임계점
...
export function getSide(pred) {
    ...
    console.log("hip");
    if (left_hip != undefined && right_hip === undefined) return "right";
    else if (left_hip === undefined && right_hip != undefined) return "left";
    ...
    if (left_shoulder != undefined && right_shoulder === undefined) return "right";
    else if (left_shoulder === undefined && right_shoulder != undefined) return "left";
    ...
    console.log("ear");
    if (left_ear === undefined && right_ear != undefined) return "right";
    else if (left_ear != undefined && right_ear === undefined) return "left";
...
export function func1(pred, side) {
    ...
    if (side == "right") {
        ...
        return getAngle(right_hip, nose, right_ear);
    }
    // 왼쪽: 17-0-11
    ...
    return getAngle(left_ear, nose, left_hip);
}
/**
 * 허리 숙인 각 계산
 * @param {JSON.predictions} pred 
...
export function func2(pred, side) {
    ...
    if (side == "right") {
        ...
        return getAngle(right_knee, right_hip, neck);
    }
    // 왼쪽: 1-11-12
    ...
    return getAngle(neck, left_hip, left_knee);
}
/**
 * 다리 꼬았는지 여부 반환
 * @param {JSON.predictions} pred 
...
export function func3(pred, side) {
    ...
    if (side == "right") {
        if (left_knee === undefined) return false;
        if (right_knee === undefined || left_knee.score > right_knee + CMP_THRESHOLD) {
            return true;
        }
        if (left_ankle === undefined) return false;
...
export function func4(pred, side) {
    ...
    if (func2(pred, side) != undefined) return undefined;
    // 오른쪽: 8-9-10
    if (side == "right") {
        ...
        return getAngle(right_hip, right_knee, right_ankle);
    }
    // 오른쪽: 13-12-11
    ...
    return getAngle(left_ankle, left_knee, left_hip);
}
